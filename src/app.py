...
from scikit_func import load_data, prepare_dataset, get_sidebar_classifier, trigger_classifier, get_model
from pyspark_func import get_spark_session, prepare_dataset_spark, get_model as get_pyspark_model, training
...
def main():
    ...
    correlation_matrix_plotly = px.imshow(data.corr(), color_continuous_scale='Viridis')
    ...
    accuracy, feature_importances, fpr, tpr, thresholds, auc, confusion_mat = classifier_results
    ...
    with col_plot3:
        ...
        if feature_importances is not None:
            ...
            features_data = pd.DataFrame({'Feature': data.columns[:-1], 'Importance': feature_importances.tolist()})
    ...
    with col_plot4:
        ...
        ''')
        if classifier_type in ['Decision Tree', 'Random Forest']:
            st.write(f"PySpark AUC: {pyspark_auc}")

            if pyspark_feature_importances is not None:
                st.subheader("Feature Importances")
                pyspark_features_data = pd.DataFrame({'Feature': data.columns[:-1], 'Importance': pyspark_feature_importances})
